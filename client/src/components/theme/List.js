...
import { list, reset } from '../../actions/theme/list';
import { success } from '../../actions/theme/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['categories'] ? itemToLinks(item['categories']) : ''}</td>
            <td>{item['versions'] ? itemToLinks(item['versions']) : ''}</td>
            <td>
...
const mapStateToProps = (state) => {
  return {
    data: state.theme.list.data,
    error: state.theme.list.error,
    loading: state.theme.list.loading,
    deletedItem: state.theme.del.deleted,
  };
};
