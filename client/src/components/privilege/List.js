...
import { list, reset } from '../../actions/privilege/list';
import { success } from '../../actions/privilege/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['type'] ? itemToLinks(item['type']) : ''}</td>
            <td>{item['group'] ? itemToLinks(item['group']) : ''}</td>
            <td>{item['subject'] ? itemToLinks(item['subject']) : ''}</td>
            <td>
...
const mapStateToProps = (state) => {
  return {
    data: state.privilege.list.data,
    error: state.privilege.list.error,
    loading: state.privilege.list.loading,
    deletedItem: state.privilege.del.deleted,
  };
};
