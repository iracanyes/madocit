...
import { list, reset } from '../../actions/abuse/list';
import { success } from '../../actions/abuse/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['accuser'] ? itemToLinks(item['accuser']) : ''}</td>
            <td>{item['defendant'] ? itemToLinks(item['defendant']) : ''}</td>
            <td>{item['chat'] ? itemToLinks(item['chat']) : ''}</td>
            <td>{item['sanction'] ? itemToLinks(item['sanction']) : ''}</td>
            <td>
...
const mapStateToProps = (state) => {
  return {
    data: state.abuse.list.data,
    error: state.abuse.list.error,
    loading: state.abuse.list.loading,
    deletedItem: state.abuse.del.deleted,
  };
};
