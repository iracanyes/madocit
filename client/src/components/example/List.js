...
import { list, reset } from '../../actions/example/list';
import { success } from '../../actions/example/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        <thead>
          <tr>
            ...
            <th>dateModified</th>
            <th>pdf</th>
            <th>associatedArticles</th>
            <th>associatedGrains</th>
            <th>video</th>
            <th colSpan={2}></th>
          </tr>
        </thead>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['associatedArticles'] ? itemToLinks(item['associatedArticles']) : ''}</td>
            <td>{item['associatedGrains'] ? itemToLinks(item['associatedGrains']) : ''}</td>
...
const mapStateToProps = (state) => {
  return {
    data: state.example.list.data,
    error: state.example.list.error,
    loading: state.example.list.loading,
    deletedItem: state.example.del.deleted,
  };
};
