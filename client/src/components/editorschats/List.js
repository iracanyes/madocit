...
import { list, reset } from '../../actions/editorschats/list';
import { success } from '../../actions/editorschats/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'][0].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['nbUnreadMessages'] ? itemToLinks(item['nbUnreadMessages']) : ''}</td>
            <td>{item['editor'] ? itemToLinks(item['editor']) : ''}</td>
            <td>{item['chatroom'] ? itemToLinks(item['chatroom']) : ''}</td>
            <td>
...
const mapStateToProps = (state) => {
  return {
    data: state.editorschats.list.data,
    error: state.editorschats.list.error,
    loading: state.editorschats.list.loading,
    deletedItem: state.editorschats.del.deleted,
  };
};
