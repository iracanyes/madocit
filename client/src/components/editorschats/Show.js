...
import {retrieve, reset} from '../../actions/editorschats/show';
  ...
  render() {
    ...
    return <div>
      ...
      {item && <table className="table table-responsive table-striped table-hover">
          ...
          <tbody>
            <tr>
              <th scope="row">nbUnreadMessages</th>
              <td>{item['nbUnreadMessages']}</td>
            </tr>
            <tr>
      ...
      {item && <Link to={`/editors_chats/edit/${encodeURIComponent(item['@id'])}`}>
...
const mapStateToProps = (state) => {
  return {
    error: state.editorschats.show.error,
    loading: state.editorschats.show.loading,
    retrieved:state.editorschats.show.retrieved,
