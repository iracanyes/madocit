...
import { list, reset } from '../../actions/note/list';
import { success } from '../../actions/note/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['moderator'] ? itemToLinks(item['moderator']) : ''}</td>
...
const mapStateToProps = (state) => {
  return {
    data: state.note.list.data,
    error: state.note.list.error,
    loading: state.note.list.loading,
    deletedItem: state.note.del.deleted,
  };
};
