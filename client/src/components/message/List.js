...
import { success } from '../../actions/message/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['attachmentFile'] ? itemToLinks(item['attachmentFile']) : ''}</td>
            <td>{item['attachmentImage'] ? itemToLinks(item['attachmentImage']) : ''}</td>
            <td>{item['attachmentUrl'] ? itemToLinks(item['attachmentUrl']) : ''}</td>
...
const mapStateToProps = (state) => {
  return {
    data: state.message.list.data,
    error: state.message.list.error,
    loading: state.message.list.loading,
    deletedItem: state.message.del.deleted,
  };
};
