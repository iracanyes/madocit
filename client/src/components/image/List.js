...
import { success } from '../../actions/image/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['place'] ? itemToLinks(item['place']) : ''}</td>
            <td>{item['title'] ? itemToLinks(item['title']) : ''}</td>
            <td>{item['url'] ? itemToLinks(item['url']) : ''}</td>
            <td>{item['alt'] ? itemToLinks(item['alt']) : ''}</td>
            <td>{item['size'] ? itemToLinks(item['size']) : ''}</td>
            <td>{item['user'] ? itemToLinks(item['user']) : ''}</td>
            <td>{item['category'] ? itemToLinks(item['category']) : ''}</td>
...
const mapStateToProps = (state) => {
  return {
    data: state.image.list.data,
    error: state.image.list.error,
    loading: state.image.list.loading,
    deletedItem: state.image.del.deleted,
  };
};
